...
require_once 'C://xampp/htdocs/mekelti2/Model/lieus.php';
...
class lieuC
{
    ...
    {
        $sql = "INSERT INTO lieuu(nomlieu,imagelieu,descriptionlieu,datelieu,datelieuf,nome) 
			VALUES (:nomlieu,:imagelieu,:descriptionlieu,:datelieu,:datelieuf,:nome)";
    ...
    public function modifierlieu($lieu, $id)
    {
        try {
            $db = config::getConnexion();
            $query = $db->prepare(
 ...
 /*   function recupererlieu($id)
    {
        $sql = "SELECT * from  lieuu L,evenment E  where L.idlieu=$id and L.nome=E.nomevent";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
            $query->execute();
